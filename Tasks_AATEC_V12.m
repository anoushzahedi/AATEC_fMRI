...
InstructPath = fullfile(currentDir, 'Pictures','instructs');
...
try
    ...
    while getParLan
        fprintf('\n****************************\n\t')
        fprintf('Please choose either DE for German or EN for English.\n\t')
        Language  = input('Language: ','s');
        fprintf('****************************\n')
        if strcmp(Language,'DE') || strcmp(Language,'EN')
            getParLan = false;
        else
            fprintf('Please choose either DE for German or EN for English.\n')
            fprintf('****************************\n')
    ...
    if strcmp(Language,'DE')
        InstructPath = fullfile(InstructPath, 'instructions_DE');
    else
        InstructPath = fullfile(InstructPath, 'instructions_EN');
    end
    ...
    while getParSes
        ...
        if session == 1 || session == 2 || session == 3
            ...
            fprintf('Please enter either 1, 2, or 3\n')
            fprintf('****************************\n')
        end
    end
    ...
    if debug
        ...
        while getFMRICheck
            fprintf('\n****************************\n\t')
            fMRI_check  = input('fMRI[J, N]: ', 's');
            fprintf('****************************\n')
            if strcmp(fMRI_check, 'J') || strcmp(fMRI_check, 'N')
                getFMRICheck = false;
            else
                fprintf('Please enter either J or N\n')
                fprintf('****************************\n')
            end
        end

        if strcmp(fMRI_check, 'J')
            fmri = true;
        elseif strcmp(fMRI_check, 'N')
            fmri = false;
        end
    else
    ...
    while getParNum
        fprintf('\n****************************\n\t')
        participantNum = input('Subject ID: ','s');
        fprintf('****************************\n')
        participantNumInt = str2num(participantNum);
        
        if  ~isempty(participantNumInt) && participantNumInt <= 999999 && participantNumInt > 99999
            if session == 1 &&  ~isfolder(fullfile(resultsDir, ['Participant_', participantNum]))
                getParNum = false;
                if ~exist(fullfile(resultsDir, ['Participant_', participantNum]),'dir')
                    mkdir(fullfile(resultsDir, ['Participant_', participantNum]))
                end
            elseif session == 2 && isfile(fullfile(resultsDir, ['Participant_', participantNum], ['logFile_',participantNum,'_2','.mat']))
                error('This session of this particiapnts is already recorded. Please check again and start over!\n')
            elseif session == 3 && isfile(fullfile(resultsDir, ['Participant_', participantNum], ['logFile_',participantNum,'_3','.mat']))
                error('This session of this particiapnts is already recorded. Please check again and start over!\n')
            elseif session == 2 || session == 3
                if isfile(fullfile(resultsDir, ['Participant_', participantNum], ['color_',participantNum,'.mat']))
                    getParNum = false;
                else
                    error('The color vector is not recorded correctly. Please check the first session and start over!')
                end
                if isfile(fullfile(resultsDir, ['Participant_', participantNum], ['dirImages_',participantNum,'.mat']))
                    getParNum = false;
                else
                    error('The color vector is not recorded correctly. Please check the first session and start over!')
                end
            else
                warning('Check the session and participants information again!')
                error('This number is already in use!')
                fprintf('****************************\n')
            end
        else
                fprintf('Please enter only a 6-digit number, which does not contain any letters!\n')
                fprintf('****************************\n')
        end 
    end
...
if fmri == 0 || session == 2
    ...
    % responsekey5 = KbName('5%');
    % responsekey4 = KbName('4$');
    % responsekey3 = KbName('3#');
    % responsekey9 = KbName('9(');
end
...
if fmri
    % Flip to clear
    DrawFormattedText(window, 'This is a test and will be closed shortly!',...
        'center', 'center', white);
    ...
    while getWin
        fprintf('\n****************************\n\t')
        scr_check  = input('Is the Screen correct (y/n): ','s');
        fprintf('****************************\n')
        if strcmp(scr_check, 'n') || strcmp(scr_check, 'y')
            getWin = false;
        else
            fprintf('Please enter either Y/N\n')
            fprintf('****************************\n')
        end
    end
    if strcmp(scr_check, 'n')
        screenNumber = 1;
    end
end
...
if fmri && (session == 1 || session == 3)
    fmri_trigger = 1;
    waitForDummyVolumes = 1;
else
    fmri_trigger = 0;
    waitForDummyVolumes = 0;
end
...
if fmri_trigger
    ...
    pulseByte = lpt2pulseByte;
    portMRI = hex2dec(lpt2in);
    nDummyVolumes = 5;
...
% 0-stimulusExtra
stimulusTimeChoice = 2;
stimulusTimePreference = 3;
stimulusTimeColorTest= 4;
...
if session ==2 
    ...
    try
        ...
        ll = 1;
        img = imread(fullfile(InstructPath,'Slide2.JPG'));
        ...
        if wait && (GetSecs - timeStart) > attentionTime
            ...
            ShowCursor;
            close all
            error('Terminated by user input!! Nothing is saved!!')
            sca;
            return
        end
...
TaskOneData = struct('ID',{}, 'Session', {},'TrialID1',{},'ImageNr',{},'Color',{},...
    'YesNoPosition',{},'RT',{},'Choice',{}, 'MRTime', {});

TaskTwoData = struct('ID',{}, 'Session', {},'TrialID2',{},'ImageNr',{},'Color',{},...
    'LikertPos',{},'RT',{}, 'RTStart',{}, 'Pref',{}, 'MRTime', {});
...
for sessionNum = session
    ...
    if strcmp(Language,'EN')
        ...
        YesNoX = [xCenter + baseSize(1)/2 - 30, xCenter - baseSize(1)/2 - 15];
    end
    ...
    ZoomingDistance = 18;
    ZoomingDistanceMin = 5;
    xyZoomScale=(size(imgSize,2)/size(imgSize,1));
    ZoomingMatrix = 8*[-1*xyZoomScale,-1,1*xyZoomScale,1];
    % Rect for the box
    pen_width = 10;
    baseBoxNum = 20;
    ...
    try
        ...
        if sessionNum == 1 || sessionNum == 3
            ...
            % Dummy-Volumes abwarten
            if waitForDummyVolumes
                rm_waitForDummyVolumes( nDummyVolumes, portMRI, pulseByte );
                waitForDummyVolumes = 0;
                % ansonsten einfach 1 Sekunde abwarten
            else
                WaitSecs(5);
            end

            % t0
            tStartExperiment = GetSecs;
            

            % Text with attention time
            AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide1.jpg'), waitVarLong, 4);
            Screen('Close');
            % fixation cross
            Screen('DrawLines', window, allCoords,...
                lineWidthPix, white, [xCenter yCenter], 2);
            ...
            % Text with attention time
            if sessionNum == 1
                AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide3.jpg'), waitVarLong);
            elseif sessionNum == 3
                AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide19.jpg'), waitVarLong);
            end
            Screen('Close');
            % fixation cross
            Screen('DrawLines', window, allCoords,...
            ...
            for Block = startingBlock:iBlocks
                ...
                if Block == 1 || Block == 3 || Block == 5 || Block == 7
                    ...
                    isiTimeSecs =  isi_general + mri_tr*rand(numTrials,1); isiTimeFrames = round(isiTimeSecs / ifi);
                    % fixation cross
                    Screen('DrawLines', window, allCoords,...
                        lineWidthPix, white, [xCenter yCenter], 2);
                    vbl = Screen('Flip', window, vbl + (waitframes - 0.5) * ifi);
                    ...
                    if Block == 3 && sessionNum == 3
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide7.jpg'), waitVarLong/2);
                    elseif Block == 1
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide4.jpg'), waitVarLong/2);
                    elseif Block == 3 && sessionNum == 1
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide5.jpg'), waitVarLong/2);
                    elseif Block == 5 || Block == 7
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide6.jpg'), waitVarLong/2);
                    end
                    Screen('Close');
                    % fixation cross
                    Screen('DrawLines', window, allCoords,...
                    ...
                    for trial= 1: numTrials
                        ...
                        timeStart = GetSecs;
                        % getting the initial position of mouse
                        [~, my_int, ~, ~] = WinJoystickMex(joy_num);
                        % timing information to start the trial
                        ...
                        while respToBeMade && (GetSecs - timeStart) < stimulusTimeChoice
                            ...
                            [~, my, ~, ~] = WinJoystickMex(joy_num);
                            if my > my_int + joy_num_y
                                Choice = 'Yes';
                                RT = GetSecs - timeStart;
                                respToBeMade = false;
                            elseif my < my_int - joy_num_y
                                Choice = 'No';
                                RT = GetSecs - timeStart;
                                respToBeMade = false;
                            end
                        ...
                        if Block ~= 1
                            ...
                            TaskOneData(trialsub).MRTime = timeStart - tStartExperiment;
                        end
                        %----------------------------------------------------------------------
                        %                           Attention test
                        %----------------------------------------------------------------------
                ...
                elseif Block == 2 || Block == 4 || Block == 6 || Block == 8
                    ...
                    % Text with attention time
                    if Block == 2
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide8.jpg'), waitVarLong/2);
                    elseif Block == 4
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide9.jpg'), waitVarLong/2);
                    elseif Block == 6 || Block == 8
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide10.jpg'), waitVarLong/2);
                    end
                    Screen('Close');
                    % fixation cross
                    Screen('DrawLines', window, allCoords,...
                    ...
                    for trial= 1: numTrials
                        ...
                        sx = (xPosScaleLines(1)+ xPosScaleLines(end))/2;
                        ...
                        respToBeMade = true;
                        responding = false;
                        % finding the initial position of mouse
                        [mx_int, ~, ~, ~] = WinJoystickMex(joy_num);
                        % timing information
                        timeStart = GetSecs;
                        WaitSecs(waitVarShort);
                        RestrictKeysForKbCheck([]);
                        ...
                        lastsecs = [];
                        mx_changed = false;
                        while (respToBeMade && (GetSecs - timeStart) < stimulusTimePreference) || responding
                            % Get the current position of the mouse
                            [mx, ~, ~, ~] = WinJoystickMex(joy_num);
                            % sliding the red cox based on the cursor's
                            % psoition
                            if mx > mx_int + 500 || mx <mx_int - 500
                                mx_change = (mx - mx_int)/joy_num_x_div;
                                mx_changed = true;
                                if sx + mx_change > xCenter + scaleHLengthPix
                                    sx = xCenter + scaleHLengthPix;
                                elseif sx + mx_change < xCenter - scaleHLengthPix
                                    sx = xCenter - scaleHLengthPix;
                                else
                                    sx = sx + mx_change;
                                    respToBeMade = false;
                        ...
                        if respToBeMade
                            ...
                            RTStart = firstsecs - timeStart;
                            RT = lastsecs - timeStart;
                            Pref = Preftmp;
                        end
                        Screen('Close');
                        % Maanaging logfiles
                        if Block ~= 2
                            ...
                            TaskTwoData(trialsub).RTStart = RTStart;
                            TaskTwoData(trialsub).Pref = Pref;
                            TaskTwoData(trialsub).MRTime = timeStart - tStartExperiment;
                        end
            ...
            if sessionNum == 3
                for Block = 1:4
                    ...
                    % Text with attention time
                    if Block == 1
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide20.jpg'), waitVarLong/3);
                    elseif Block == 2 || Block == 3 || Block == 4
                        AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide21.jpg'), waitVarLong/3);
                    end
                    Screen('Close');
                    % fixation cross
                    Screen('DrawLines', window, allCoords,...
                    ...
                    for trial= 1: numTrials
                        ...
                        while respToBeMade && (GetSecs - timeStart) < stimulusTimeChoice 
                            ...
                            [mx, ~, ~, ~] = WinJoystickMex(joy_num);
                            if mx - mx_int > joy_num_x
                                Choice = 'right';
                                RT = GetSecs - timeStart;
                                respToBeMade = false;
                            elseif mx - mx_int < -joy_num_x
                                Choice = 'left';
                                RT = GetSecs - timeStart;
                                respToBeMade = false;
                            end
                        ...
                        if Block ~= 1
                            ...
                            TaskThreeData(trialsub).MRTime = timeStart - tStartExperiment;
                        end
        ...
        elseif sessionNum == 2
            ...
            % Text with attention time
            AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide11.jpg'), waitVarLong);
            Screen('Close');
            % fixation cross
            Screen('DrawLines', window, allCoords,...
                lineWidthPix, white, [xCenter yCenter], 2);
            ...
            for Block = Block_order
                ...
                % Text with attention time
                if Block == 1
                    AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide12.jpg'), waitVarLong/2);
                elseif Block == 2
                    AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide16.jpg'), waitVarLong/2);
                elseif (strcmp(Block_order_var,'1_2') && Block == 3) || (strcmp(Block_order_var,'2_1') && Block == 4)
                    AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide18.jpg'), waitVarLong/2);
                elseif Block == 3 || Block == 5
                    AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide15.jpg'), waitVarLong/2);
                elseif Block == 4 || Block == 6
                    AttentionTestFailed = general_attention(true,fullfile(InstructPath,'Slide17.jpg'), waitVarLong/2);
                end
                Screen('Close');
                % fixation cross
                Screen('DrawLines', window, allCoords,...
                ...
                if Block == 1
                    ...
                    img = imread(fullfile(InstructPath,'Slide13.JPG'));
                    target_height = screenYpixels;
                    target_size = [target_height NaN];
                    img = imresize(img, target_size);
                    Texture = Screen('MakeTexture', window, img);
                    ...
                    while wait
                        ...
                        [~, my, ~, ~] = WinJoystickMex(joy_num);
                        if my > my_int + joy_num_y_training
                            wait = false;
                        end
                    ...
                    Screen('DrawLines', window, allCoords,...
                        lineWidthPix, white, [xCenter yCenter], 2);
                    vbl = Screen('Flip', window, vbl + (waitframes - 0.5) * ifi);
                    WaitSecs(waitVarLong);

                    % Second sound training
                    img = imread(fullfile(InstructPath,'Slide14.JPG'));
                    target_height = screenYpixels;
                    target_size = [target_height NaN];
                    img = imresize(img, target_size);
                    Texture = Screen('MakeTexture', window, img);
                ...
                for trial = trial_replace
                    ...
                    if strcmp(tmp.LearningMethod, 'AAT')
                        ...
                        recttmp = rect;
                        rectBoxtmp = rectBox;
                        while respToBeMade && (GetSecs - timeStartBeep) < (waitLearningAAT + ExtraTimeAAT*ifi) && counterlocal < ZoomingDistance || (responding && (GetSecs - timeStartBeep) < (waitLearningAAT + ExtraTimeAAT*ifi)) 
                            % Check the keyboard. The person should press either
                            ...
                            if my - my_int > 1000
                                ...
                                respToBeMade = false;
                                if strcmp(tmp.LearningValence,'p')
                                    if counterlocal < ZoomingDistance
                            ...
                            if ~isempty(Response) && ~max_reached
                                if GetSecs - lastsecs < 0.15
                                    responding = true;
                                else
                                    responding = false;
                                end
                        ...
                        elseif ~respToBeMade && counterlocal > 0 && counterlocal < ZoomingDistanceMin
                            Accuracy = false;
                            % Punishment Image
                            img = imread(fullfile(ImgEmoPath,'StopSign', 'Stop.png'));
                            Texture = Screen('MakeTexture', window, img);
                    ...
                    elseif strcmp(tmp.LearningMethod, 'EC')
                        ...
                        respToBeMade = true;
                        while respToBeMade && (GetSecs - timeStartEmo) < (waitLearningEC + ExtraTimeEC*ifi)
                            % Check the keyboard. The person should press either
                            % 'm' or 'c' to accept or reject the picture
                            [keyIsDown,secs, keyCode] = KbCheck;
                            if keyCode(EscapeKey) && debug_key
                            ...
                            [~,~,~,buttons] = WinJoystickMex(joy_num);
                            if buttons(1)
                                Response = 'Positive';
                                if strcmp(tmp.LearningValence,'p')
                                    Accuracy = true;
        ...
        RestrictKeysForKbCheck(qKey);
        wait = true;
        timeStart = GetSecs;
        while wait && GetSecs - timeStart < attentionTimeLong
            [~,~, keyCode] = KbCheck;
            if keyCode(qKey)
                wait = false;
            end
